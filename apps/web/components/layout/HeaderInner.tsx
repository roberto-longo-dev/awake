...
export function HeaderInner({ user }: HeaderInnerProps) {
  ...
  return (
    <div ref={wrapperRef}>
      <div className="relative max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        {/* Left: burger on mobile, nothing on desktop */}
        <button
        ...
        >
          <span
            className={`block w-5 h-0.5 bg-text transition-all duration-300 ease-in-out ${
              isMenuOpen ? "translate-y-[8px] rotate-45" : "translate-y-0 rotate-0"
            }`}
          />
          <span
            className={`block w-5 h-0.5 bg-text transition-all duration-300 ease-in-out ${
        ...
        {/* Right side: user menu or login icon + Order button */}
